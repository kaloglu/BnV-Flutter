...
export 'package:BedavaNeVar/data/services/interfaces/db_service.dart';

class FirestoreDBService implements DBService {
  final FirebaseFirestore _firestore;

  FirestoreDBService({FirebaseFirestore firestore}) : _firestore = firestore ?? FirebaseFirestore.instance;
  ...
  enroll(Enroll enroll, String uid) {
    ...
    _firestore.runTransaction((Transaction tx) async {
      ...
      if (ticketSnapshot.exists) {
        ...
        tx.update(ticketRef, {"remain": ticket.remain - 1});
  ...
  rewardTicket(int count, String source, String uid) {
    _firestore.runTransaction((Transaction tx) async {
      var documentRef = getTicketCollection(uid).doc();
      await getTicketCollection(uid)
          .doc(documentRef.id)
          .set(Ticket(id: documentRef.id, source: source, userId: uid, earn: count).toJson());
    });
  }
  ...
  DocumentReference getRaffleReference(String raffleId) => getRaffleCollection.doc(raffleId);
  ...
  DocumentReference getTicketReference(String ticketId, String uid) => getTicketCollection(uid).doc(ticketId);
  ...
  Future<QuerySnapshot> getUnregisteredDeviceTokens(String deviceToken) =>
      getUnRegisteredDeviceTokensCollection.where("deviceToken", isEqualTo: deviceToken).get();

  DocumentReference getUnregisteredToken(String token) => getUnRegisteredDeviceTokensCollection.doc(token);

  @override
  Future<User> getUser([userId]) async => Future.value(User.fromFirestore(await getUserReference(userId).get()));

  DocumentReference getUserReference(String uid) => getUserCollection.doc(uid);
  ...
  Future<void> saveToken(String token, {String uid = ""}) async {
    ...
    if (uid != null && uid.isNotEmpty) {
      ...
      getUnregisteredDeviceTokens(token).then((result) {
        if (result.docs.length <= 0) getUnRegisteredDeviceTokensCollection.doc().set(deviceToken);
      });
    }
  }
  ...
  Future<void> userCreateOrUpdate(User user) async {
    ...
    _firestore.runTransaction((Transaction tx) async {
      DocumentSnapshot userSnapshot = await tx.get(userRef);
      if (userSnapshot.exists) {
        tx.update(userRef, User.fromFirestore(userSnapshot).toJson());
      } else {
